...
// Create a member card component with improved styling
const MemberCard: React.FC<{ member: FamilyMember; isSpouse?: boolean }> = ({ member, isSpouse = false }) => {
  return (
    <motion.div 
      className={`bg-gray-800 text-white p-4 rounded-lg shadow-lg flex flex-col items-center min-w-48 ${isSpouse ? 'border-l-2 border-gray-600' : ''}`}
    ...
    >
      ...
      <p className="font-bold text-center mb-1">{member.name}</p>
    </motion.div>
  );
};
...
const Home: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen bg-gray-900 flex flex-col">
      <h1 className="text-4xl font-bold text-white text-center py-6">Royal Family Tree</h1>
